...
func Default() Config {
	...
	return Config{
		...
		Oracle: OracleConfig{
			// Canonical Chainlink aggregator addresses on Ethereum mainnet.
			// Override per-chain in config.yaml (`oracle.chainlink_feeds`) when
			// reading prices on another network.
